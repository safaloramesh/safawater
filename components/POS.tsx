            ...
            <div className="flex-1 overflow-y-auto p-4 flex justify-center bg-slate-100 dark:bg-slate-900">
              <div id="printable-receipt" className="print-area bg-white text-black p-4 w-[80mm] shadow-lg">
                ...
                <div className="thermal-small leading-loose">
                  <div className="flex justify-between"><span>Inv No:</span><span className="thermal-bold">{showReceipt.id}</span></div>
                  <div className="flex justify-between"><span>Date:</span><span>{new Date(showReceipt.timestamp).toLocaleString([], {hour: '2-digit', minute:'2-digit', day:'2-digit', month:'2-digit', year:'numeric'})}</span></div>
                  <div className="flex justify-between"><span>Status:</span><span className="thermal-bold uppercase">{showReceipt.paymentMethod}</span></div>
                </div>
                ...
                <div className="thermal-small leading-tight mb-2">
                  ...
                  <div className="flex justify-between"><span>TEL:</span><span>{showReceipt.customerPhone}</span></div>
                </div>
                ...
                <div className="thermal-small leading-tight mb-2">
                  ...
                  <div className="thermal-small opacity-80 pl-2">
                    Rate: {showReceipt.rate.toFixed(3)} BHD
                  </div>
                </div>

                <div className="thermal-small leading-tight italic opacity-70 mb-2">
                  <div className="flex justify-between"><span>Pump:</span><span>#0{showReceipt.pumpId}</span></div>
                  <div className="flex justify-between"><span>Begin:</span><span>{showReceipt.startReading.toFixed(2)}</span></div>
                  <div className="flex justify-between"><span>End:</span><span>{showReceipt.endReading.toFixed(2)}</span></div>
                </div>

                <div className="thermal-divider"></div>

                <div className="flex justify-between items-end mb-4">
                  <span className="thermal-bold uppercase">Grand Total:</span>
                  <div className="thermal-right">
                    <p className="thermal-bold thermal-large">{showReceipt.totalAmount.toFixed(3)} <span className="thermal-small">BHD</span></p>
                  </div>
                </div>
                ...
                <div className="thermal-center mt-6">
                  <p className="thermal-small italic">Thank you for your business!</p>
                  <p className="thermal-small thermal-bold mt-2 uppercase">{state.systemInfo.companyName} - QUALITY FIRST</p>
                </div>
                
                <div className="mt-8 thermal-center">
                   <div className="border-t border-black w-3/4 mx-auto pt-1">
                      <p className="thermal-small uppercase">Authorized Sign</p>
                   </div>
                </div>
                
                {/* Spacing for paper cut */}
                <div className="h-12"></div>
              </div>
            </div>

            <div className="p-6 bg-slate-50 dark:bg-slate-800 flex gap-4 no-print border-t border-slate-100 dark:border-slate-700">
              <button onClick={() => window.print()} className="flex-1 bg-slate-900 dark:bg-blue-600 text-white font-black py-5 rounded-2xl flex items-center justify-center gap-3 transition-all hover:bg-black dark:hover:bg-blue-700 active:scale-95"><Printer size={24} />PRINT TICKET</button>
